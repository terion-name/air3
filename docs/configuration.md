...
NATS is a control plane only. air3 uses NATS Core publish/subscribe and queue groups; it does not use JetStream and does not send object bytes over NATS.
...
| `broker` | `edge-gateway`, `nats`, `private-connector` | NATS Core control plane and edge ingest reachability. |
...
- `nats` uses `deploy/nats/nats.conf`, TLS/mTLS, username/password auth, and NATS Core only.
